...
namespace Laba2
{
    internal class Product
    {
        ...
        {
            if (CounterProduct >= MaxProduct)
            {
                Console.WriteLine("Добавление дополнительных продуктов на полку невозможно");
            }
            else
            {
                CounterProduct++;
        ...
        public static Product AddProduct()
        {
            ...
            int lenght_struct = Print.GetIntInput("Сколько ингридиентов в составке продукта?");
        ...
        public static void RemoveProduct(Product[] allProducts)
        {
            string search_product_for_remove = Print.GetInput("Введите название продукта, который необходимо удалить");

            Product productToRemove = Array.Find(allProducts, product =>
            product != null && product.Name != null && product.Name.ToLower() == search_product_for_remove.ToLower());

            if (productToRemove != null)
            {
                Array.Clear(allProducts, Array.IndexOf(allProducts, productToRemove), 1);
        ...
        public static void PrintAllProduct(Product[] allProducts)
        {
            int count = 1;
            if (allProducts != null)
            {
        ...
        public static void SearchLinesInSelectedProduct(Product[] AllProducts, int selected)
        {
        ...
        public static Product SearchSelectedProduct(Product[] allProducts)
        {
            string current_name_product = Print.GetInput("Введите название продукта для его изменения");

            Product productToRemove = Array.Find(allProducts, product =>
            product != null && product.Name != null && product.Name.ToLower() == current_name_product.ToLower());
            if (productToRemove != null)
            {
